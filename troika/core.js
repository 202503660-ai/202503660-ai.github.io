...
const slotHours=[6,5,3,3,4,3];
function share(p,step){const sum=p.timeFlow.reduce((a,b)=>a+(b||0),0);return sum?p.timeFlow[clampStep(step)]/sum:null;}
// 여섯 구간의 길이가 6·5·3·3·4·3시간으로 다르다. 원자료 구성비를 그대로 쓰면
// 가장 긴 00–06시가 항상 최대로 보인다. 시간당으로 환산해 비교 가능하게 만든다. (TROIKA-B10-12 7.1)
function sharePerHour(p,step){const per=p.timeFlow.map((v,i)=>(v||0)/slotHours[i]);const sum=per.reduce((a,b)=>a+b,0);return sum?per[clampStep(step)]/sum:null;}
...
const c={defaults,clampStep,slotHours,timeLabel:s=>slots[clampStep(s)],share,sharePerHour,simulation,conceptLabels,concepts,matchingConcept,score,rank,normalizeStamps,addStamp,readStamps};
