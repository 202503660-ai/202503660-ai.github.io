(() => {
  ...
  function changeTime(step) {
    ...
    $('timeHint').textContent = '통계 분포 + 가상 체험';
  ...
  function allocationMarkup(p){
    const a=p.allocation;
    if(!a)return `<section class="allocation-evidence"><h3>스탬프 배정안 · 0개</h3><p>연계존은 배정 대상에서 제외하고 교통 연계를 검토합니다.</p><p class="field-hint">확정 제휴나 교통 서비스가 아닌 운영 제안입니다.</p></section>`;
    return `<section class="allocation-evidence"><p class="eyebrow">2026년 2분기 · 정책 배정 근거</p><h3>스탬프 ${p.plannedQuota}개 · 소외도 ${a.M.toFixed(3)}</h3><p>최근 하락과 낮은 점포당 매출, 거리 기반 도달을 함께 고려합니다.</p><div class="allocation-values"><span>D 하락 <strong>${a.D.toFixed(3)}</strong></span><span>L 저활력 <strong>${a.L.toFixed(3)}</strong></span><span>R 도달 <strong>${a.R.toFixed(3)}</strong></span></div><p><strong>M = √(D × L) × R</strong><br>기존 ${a.previousQuota}개에서 ${p.plannedQuota}개로 · 최소 2개 / 이 상권 상한 ${a.cap}개</p><p class="field-hint">상한은 6개와 집계 점포 수의 20%를 내림한 값 중 작은 값입니다. 취향 추천 점수와는 다른 정책 지수입니다.</p>${a.imputed?'<p class="allocation-note">이경시장 매출은 직전 유효분기인 2026년 1분기 값으로 대체했습니다. 점포당 매출은 현재 점포 수로 나눈 값입니다.</p>':''}<details><summary>지표와 출처</summary><p>D: 20대 소비강도 변화·점포 수 변화·폐업률·전체 추정매출 변화를 백분위화해 평균한 뒤 다시 백분위화합니다.</p><p>L: 점포당 추정매출의 역순위 백분위. 소비강도는 개인별 객단가가 아닌 대리 지표입니다.</p><p>R: exp(−1.189 × 거리[km]). 무대 대리점에서 상권 경계까지의 거리이며 카드의 권역 중심 직선거리와 다릅니다.</p><p>B10-14 배정표 7–8쪽의 소수 셋째 자리 공개값입니다. 원본 거리로 재계산한 값이 아니므로 표시값으로 계산한 M과 반올림 차이가 날 수 있습니다.</p></details><p class="field-hint">16개 상권 총 60개 모집 배정안 · 확정 제휴 매장 수가 아닙니다.</p></section>`;
  }
  ...
  function openDetail(id,opener){currentPlace=D.places.find(p=>p.id===id);if(!currentPlace)return;const p=currentPlace;$('detailRegion').textContent=`${regionText(p)} / 상권 코드 ${p.id}`;$('detailTitle').textContent=p.name;$('detailDescription').textContent=`2026년 2분기 서울시 상권 자료. 기획 테마: ${p.theme}.`;
    renderDetailMeta();
    $('storeList').innerHTML=`${allocationMarkup(p)}<div class="concept-tags">${conceptMarkup(p)}</div><p class="field-hint">업종 집계로 구성한 탐방 컨셉입니다. 분위기·시설·개별 매장 품질을 확인한 정보는 아닙니다.</p><button class="button primary" data-stamp="${p.id}">탐방 목록에 저장</button><div class="demo-detail">${simMarkup(p)}<p>축제 상황을 가정한 체험용 수치입니다.</p><button class="button primary" data-demo="${p.id}">가상 스탬프 받기</button></div><h3>시간대별 유동인구 분포</h3><p class="field-hint">시간 구간 길이가 달라요. 각 구간 원자료의 구성비이며 혼잡도 비교 지표가 아닙니다.</p><div class="flow-bars">${p.timeFlow.map((v,i)=>`<div><span>${C.timeLabel(i)}</span><meter min="0" max="1" value="${C.share(p,i)||0}" aria-label="${C.timeLabel(i)} 비중"></meter><span>${pct(C.share(p,i))}</span></div>`).join('')}</div><details open><summary>탐방 관련 업종별 점포 수</summary><p class="field-hint">음식·카페·장보기·쇼핑 업종만 표시합니다. 전체 집계 점포 수와 합계가 다를 수 있어요.</p><table class="sector-table"><thead><tr><th>업종</th><th>점포 수</th></tr></thead><tbody>${p.sectors.filter(s=>s.count>0).map(s=>`<tr><td>${escape(s.name)}</td><td>${fmt(s.count)}</td></tr>`).join('')}</tbody></table></details>`;
    $('detailDialog').querySelector('.dialog-notice').textContent=`기획 구분: ${p.zone} · 스탬프 모집 배정안 ${p.plannedQuota}개. 확정 가맹점·쿠폰 정보는 제공 자료에 없습니다.`;syncStampButtons();renderDemo();openDialog($('detailDialog'),opener);
  }
  ...
  function renderStamps(){ $('stampCount').innerHTML=stamps.length+' <span>/ 3</span>';$('stampShortcut').textContent=`탐방 목록 ${stamps.length}/3`;$('stampSlots').innerHTML=[0,1,2].map(i=>`<div class="stamp-slot ${stamps[i]?'filled':''}">${icon(stamps[i]?'check':'plus')}<span>${stamps[i]?escape(storeMap.get(stamps[i]).name):`${i+1}번째 탐방 상권`}</span></div>`).join('');$('rewardStatus').textContent='최종 배정안: 코어 36개 · 참여 24개 · 연계 0개. 16개 상권 대상 모집 계획입니다.';$('rewardState').textContent='운영 기획';$('storageNotice').textContent=storageAvailable?'탐방 목록은 이 브라우저에 저장됩니다.':'브라우저 저장을 사용할 수 없어 현재 화면에만 유지됩니다.';syncStampButtons();}
